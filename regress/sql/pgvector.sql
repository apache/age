...
-- Test vector index
--
SELECT * FROM cypher('graph', $$ EXPLAIN (COSTS OFF)
                                 MATCH (m:Movie), (search:Movie {title: "The Matrix"})
...
USING hnsw (((
  agtype_access_operator(
    VARIADIC ARRAY[
      _agtype_build_vertex(id, _label_name('21968'::oid, id), properties),
      '"embedding"'::agtype
    ]
  )::text
...
SELECT * FROM cypher('graph', $$ MATCH (m:Movie)
                                 WITH m.title AS title, m.embedding::vector(4) <=> [-0.07594558, 0.04081754, 0.29592122, -0.11921061]::vector(4) AS dist
                                 RETURN title, (1 - dist) AS similarity
                                 ORDER BY dist
                                 ASC LIMIT 4
$$) AS (title agtype, similarity agtype);
SELECT * FROM cypher('graph', $$ EXPLAIN (COSTS OFF)
                                 MATCH (m:Movie)
                                 WITH m.title AS title, m.embedding::vector(4) <=> [-0.07594558, 0.04081754, 0.29592122, -0.11921061]::vector(4) AS dist
                                 RETURN title, (1 - dist) AS similarity
