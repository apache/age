...
pqSaveParameterStatus(PGconn *conn, const char *name, const char *value)
{
	...
	else if (strcmp(name, "pg_graph_version") == 0)
	{
		int			cnt;
		int			vmaj,
					vmin,
		...
		if (cnt == 3)
		{
			...
			conn->pg_graph_version = (100 * vmaj + vmin) * 100 + vrev;
		}
		else if (cnt == 2)
		{
			/* e.g. 2.2devel*/
			conn->pg_graph_version = (100 * vmaj + vmin) * 100;
		}
		else
			conn->pg_graph_version = 0; /* unknown */
	}
}
