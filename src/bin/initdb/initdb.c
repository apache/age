...
/*
 * write out the PG_VERSION / PG_GRAPH_VERSION file in the data dir, or its subdirectory
...
write_version_file(const char *extrapath)
{
	...
	/* write PG_GRAPH_VERSION */
	if (extrapath == NULL)
		path = psprintf("%s/PG_GRAPH_VERSION", pg_data);
	else
		path = psprintf("%s/%s/PG_GRAPH_VERSION", pg_data, extrapath);
...
setup_pgdata(void)
{
	...
	pggdata_set_env = psprintf("PGGDATA=%s", pg_data);
	putenv(pggdata_set_env);
}
