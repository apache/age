...
do_help(void)
{
	printf(_("%s is a utility to initialize, start, stop, or control a pg_graph server.\n\n"), progname);
	printf(_("Usage:\n"));
...
main(int argc, char **argv)
{
	...
	if (pg_config == NULL)
	{
		...
		pggdata_D = getenv("PGGDATA");

		if(pggdata_D != NULL)
		{
			canonicalize_path(pggdata_D);
			env_var = psprintf("PGDATA=%s", pggdata_D);
			putenv(env_var);
			 ...
			 * 'ps' display
			 */
			pgdata_opt = psprintf("-D \"%s\" ", pggdata_D);
			pg_config = getenv("PGDATA");
		}
	}
