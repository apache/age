...
static label_cache_data *search_label_name_graph_cache_miss(Name name,
                                                            uint32 graph_oid);
static void *label_name_graph_cache_hash_search(Name name, uint32 graph,
                                                HASHACTION action, bool *found);
static label_cache_data *search_label_graph_oid_cache_miss(uint32 graph,
                                                          int32 id);
...
static void flush_graph_namespace_cache(void)
{
    ...
    for (;;)
    {
        ...
        removed = hash_search(graph_namespace_cache_hash, &entry->nspid,
                              HASH_REMOVE, NULL);
        if (!removed)
        {
            ereport(ERROR,
                                  ...
                                  HeapTuple tuple, TupleDesc tuple_desc)
{
    ...
    Assert(!is_null);
    cache_data->id = DatumGetUInt32(value);
    // ag_graph.name
    ...
    Assert(!is_null);
    cache_data->nspid = DatumGetObjectId(value);
}
...
label_cache_data *search_label_name_graph_cache(const char *name,uint32 graph_oid)
{
    NameData name_key;
    label_name_graph_cache_entry *entry;
    ...
    namestrcpy(&name_key, name);
    entry = label_name_graph_cache_hash_search(&name_key, graph_oid, HASH_FIND,
                                               NULL);
    if (entry)
        return &entry->data;

    return search_label_name_graph_cache_miss(&name_key, graph_oid);
}
                                                            ...
                                                            uint32 graph_oid)
{
    ...
    scan_keys[0].sk_argument = NameGetDatum(name);
    scan_keys[1].sk_argument = UInt32GetDatum(graph_oid);
    ...
    // get a new entry
    entry = label_name_graph_cache_hash_search(name, graph_oid, HASH_ENTER,
                                               &found);
...
static label_cache_data *search_label_graph_oid_cache_miss(uint32 graph, int32 id)
{
    ScanKeyData scan_keys[2];
    Relation ag_label;
    SysScanDesc scan_desc;
    ...
    scan_keys[0].sk_argument = UInt32GetDatum(graph);
                                  ...
                                  HeapTuple tuple, TupleDesc tuple_desc)
{
    ...
    Assert(!is_null);
    cache_data->graph = DatumGetUInt32(value);
    // ag_label.id
