...
#define SUBMIT_FIELD(p) \
    ...
    { \
        ...
        if (cb1 && (p->options & CSV_EMPTY_IS_NULL) && !quoted && \
            entry_pos == 0) \
             ...
             void (*cb2)(int c, void *), void *data)
{
    ...
    {
        /* Current field is quoted, no end-quote was seen, and CSV_STRICT_FINI
         * is set */
        p->status = CSV_EPARSE;
        return -1;
    }
                 ...
                 void (*cb2)(int c, void *), void *data)
{
    ...
    while (pos < len)
    {
        ...
        case FIELD_NOT_BEGUN:
            ...
            else if (is_term ? is_term(c) :
                               c == CSV_CR || c == CSV_LF) /* Carriage
                                                              Return
                                                              or
                                                              Line
                                                              Feed
        ...
        case FIELD_BEGUN:
            if (c == quote) /* Quote */
            {
                ...
                else
                {
                    ...
                    {
                        p->status = CSV_EPARSE;
                        p->quoted = quoted, p->pstate = pstate,
                        p->spaces = spaces, p->entry_pos = entry_pos;
                        return pos - 1;
                    }
            ...
            break;
        case FIELD_MIGHT_HAVE_ENDED:
            /* This only happens when a quote character is encountered in a
             * quoted field */
            if (c == delim) /* Comma */
            {
            ...
            else if (is_space ? is_space(c) :
                                c == CSV_SPACE || c == CSV_TAB) /* Space or Tab
                                                                 */
            {
                SUBMIT_CHAR(p, c);
                spaces++;
