 ...
 */
/*
 * ValidatePgGraphVersion() is always called by ValidatePgVersion().
 * Because seperated call could make human mistake
 * and renaming ValidatePgVersion() can cause future merge confilct.
 */
static void
ValidatePgGraphVersion(const char *path)
{
	char		full_path[MAXPGPATH];
	FILE	   *file;
	char		file_version_string[64];

	snprintf(full_path, sizeof(full_path), "%s/PG_GRAPH_VERSION", path);
	...
	if (strcmp(PG_GRAPH_COMP_VERSION, file_version_string) != 0)
		ereport(FATAL,
				(errcode(ERRCODE_INVALID_PARAMETER_VALUE),
						errmsg("database files are incompatible with server"),
						errdetail("The data directory was initialized by pg_graph version %s, "
								  "which is not compatible with this version %s.",
								  file_version_string, PG_GRAPH_COMP_VERSION)));
}
...
ValidatePgVersion(const char *path)
{
	...
	/* PG_GRAPH_VERSION must be validated too */
	ValidatePgGraphVersion(path);
}
