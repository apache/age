...
typedef struct GRAPH_global_context
{
    ...
    uint32 graph_oid;                 /* graph oid for searching */
...
static List *get_ag_labels_names(Snapshot snapshot, uint32 graph_oid,
                                 char label_type);
...
static void load_edge_hashtable(GRAPH_global_context *ggctx)
{
    ...
    edge_label_names = get_ag_labels_names(snapshot, graph_oid, LABEL_TYPE_EDGE);
 ...
 */
GRAPH_global_context *find_GRAPH_global_context(uint32 graph_oid)
{
