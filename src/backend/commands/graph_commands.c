...
/*
 * This is call callback function
 * This function will be called when user will call SELECT drop_label.
 * The function takes two parameters
 * 1. Graph name
 * 2. Label Name
 * Function will drop a edge label
 * Function returns error if graph or label names or not provided
*/
...
Datum drop_label(PG_FUNCTION_ARGS)
{
    ...
    graph = graph_name->data;
    label = label_name->data;
    delete_label(get_label_id(label, get_graph_oid(graph)));

    ereport(NOTICE,
            (errmsg("Label \"%s\" has been deleted", NameStr(*label_name))));
