...
void insert_label(const char *label_name, uint32 graph_oid, int32 label_id,
                  char label_kind, Oid label_relation)
{
    ...
    values[Anum_ag_label_graph - 1] = UInt32GetDatum(graph_oid);
...
int32 get_label_id(const char *label_name, uint32 graph_oid)
{
...
Oid get_label_relation(const char *label_name, uint32 graph_oid)
{
    label_cache_data *cache_data;
...
char *get_label_relation_name(const char *label_name, uint32 graph_oid)
{
...
bool label_id_exists(uint32 graph_oid, int32 label_id)
{
    label_cache_data *cache_data;
 ...
 */
RangeVar *get_label_range_var(char *graph_name, uint32 graph_oid,
                              char *label_name)
{
    char *relname;
    label_cache_data *label_cache;
 ...
 */
List *get_all_edge_labels_per_graph(EState *estate, uint32 graph_oid)
{
    List *labels = NIL;
    ...
    ScanKeyInit(&scan_keys[1], Anum_ag_label_graph, BTEqualStrategyNumber,
                F_OIDEQ, UInt32GetDatum(graph_oid));
