...
%token <keyword> ABORT_P ABSOLUTE_P ACCESS ACTION ADD_P ADMIN AFTER
	...
	CONNECTION CONSTRAINT CONSTRAINTS CONTAINS CONTENT_P CONTINUE_P CONVERSION_P COPY
	...
	START STARTS STATEMENT STATISTICS STDIN STDOUT STORAGE STORED STRICT_P STRIP_P
 ...
 *
 * To support Cypher, the precedence of unreserved keywords, such as
 * ALLSHORTESTPATHS, DELETE_P, DETACH, DIJKSTRA, LOAD, OPTIONAL_P, REMOVE,
 * SHORTESTPATH, SINGLE, SIZE_P and SKIP, must be the same as that of IDENT.
 */
 ...
 *
 * CypherStmt conflict with routine_body_stmt, so, includes them as
 * toplevel_stmt instead stmt. ( e717a9a18b2e34c9c40e5259ad4d31cd7e420750 )
 */
toplevel_stmt:
			stmt
...
ReindexStmt:
			...
			| REINDEX '(' utility_option_list ')' reindex_target_label name
				{
					ReindexStmt *n = makeNode(ReindexStmt);
					n->kind = $5;
...
CreateConstraintStmt:
			CREATE opt_or_replace CONSTRAINT opt_constraint_name ON ColId ASSERT cypher_expr
			IS UNIQUE
				{
					CreateConstraintStmt *n;

					if ($2) /* not supported, see CreateTrigger */
						ereport(ERROR,
								(errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
								 errmsg("CREATE OR REPLACE CONSTRAINT is not supported")));

					n = makeNode(CreateConstraintStmt);
					n->graphlabel = makeRangeVar(NULL, $6, @6);
					n->conname = $4;
					n->contype = CONSTR_UNIQUE;
					n->expr = $8;
					$$ = (Node *) n;
...
CreatePropertyIndexStmt:
			...
			name ON ColId access_method_clause '(' prop_idx_params ')'
			opt_reloptions OptTableSpace where_clause
				{
					CreatePropertyIndexStmt *n;
...
cypher_read_stmt:
			...
			| cypher_read_opt_parens UNION set_quantifier
			  cypher_read_opt_parens
					{ $$ = makeCypherSetOp(SETOP_UNION, $3 == SET_QUANTIFIER_ALL, $1, $4); }
			| cypher_read_opt_parens INTERSECT set_quantifier
			  cypher_read_opt_parens
					{ $$ = makeCypherSetOp(SETOP_INTERSECT, $3 == SET_QUANTIFIER_ALL, $1, $4); }
			| cypher_read_opt_parens EXCEPT set_quantifier
			  cypher_read_opt_parens
					{ $$ = makeCypherSetOp(SETOP_EXCEPT, $3 == SET_QUANTIFIER_ALL, $1, $4); }
		;
...
cypher_i_expr:
			...
			| cypher_i_expr qual_Op cypher_i_expr			%prec Op
					{ $$ = (Node *) makeA_Expr(AEXPR_OP, $2, $1, $3, @2); }
			| qual_Op cypher_i_expr							%prec Op
					{ $$ = (Node *) makeA_Expr(AEXPR_OP, $1, NULL, $2, @1); }
...
cypher_w_expr:
			cypher_w_expr OR cypher_w_expr
					{ $$ = makeOrExpr($1, $3, @2); }
			...
			| cypher_w_expr NOT_EQUALS cypher_w_expr
				{
					$$ = (Node *) makeSimpleA_Expr(AEXPR_OP, "<>", $1, $3, @2);
				}
			| cypher_w_expr '<' cypher_w_expr
			...
			| cypher_w_expr LESS_EQUALS cypher_w_expr
				{
					$$ = (Node *) makeSimpleA_Expr(AEXPR_OP, "<=", $1, $3, @2);
				}
			| cypher_w_expr GREATER_EQUALS cypher_w_expr
				{
					$$ = (Node *) makeSimpleA_Expr(AEXPR_OP, ">=", $1, $3, @2);
				}
			| cypher_w_expr '+' cypher_w_expr
			...
			| cypher_w_expr '[' cypher_expr_opt DOT_DOT cypher_expr_opt ']'
				{
					A_Indices  *ind;
					A_Indirection *n;
			...
			| cypher_w_expr ENDS WITH cypher_expr			%prec ENDS
				{
					$$ = (Node *) makeFuncCall(
										SystemFuncName("string_ends_with"),
										list_make2($1, $4),
			...
			| cypher_w_expr CONTAINS cypher_expr
				{
					$$ = (Node *) makeFuncCall(
										SystemFuncName("string_contains"),
										list_make2($1, $3),
			...
			| cypher_w_expr EQUALS_TILDE cypher_expr
				{
					$$ = (Node *) makeFuncCall(
										SystemFuncName("string_regex"),
										list_make2($1, $3),
			...
			| cypher_w_expr IS NULL_P						%prec IS
				{
					NullTest   *n;
			...
			| cypher_w_expr '.' cypher_expr_name
				{
					A_Indirection *n;
			...
			| type_function_name '(' DISTINCT cypher_expr_comma_list ')'
				{
					...
					n = makeFuncCall(list_make1(makeString($1)), $4,
									 COERCE_EXPLICIT_CALL, @1);
					n->agg_distinct = true;
					$$ = (Node *) n;
				}
			| type_function_name '(' '*' ')'
				{
					...
					n = makeFuncCall(list_make1(makeString($1)), NIL,
									 COERCE_EXPLICIT_CALL, @1);
					n->agg_star = true;
					$$ = (Node *) n;
				}
