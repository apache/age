 ...
 *
 * IDENTIFICATION
 *	  src/spec/packstream/pack_structure.c
 */
...
char *write_tiny_struct_marker(size_t element_count) {
    ...
    if (element_count > PM_TINY_STRUCT_MAX) {
        perror("parameter value is exceed range of tiny_struct");
        return NULL;
    }
    result = malloc(PACK_MARKER_LENGTH);
    result[0] = (char) (PM_TINY_STRUCT | (NIBBLE_LOW & element_count));
    return result;
}