...
char *write_tiny_list_marker(int element_count) {
    ...
    if (element_count > PM_TINY_LIST_MAX) {
    ...
    char size;
    size = NIBBLE_LOW & element_count;

    result[0] = (char) (PM_TINY_LIST | (NIBBLE_LOW & element_count));
...
char *write_list8_marker(int element_count) {
    ...
    if (element_count > PM_TINY_LIST8_MAX) {
        perror("parameter value is exceed range of list8");
        return NULL;
    }

    result = malloc(PACK_MARKER_LENGTH + PACK_SIZE_LENGTH);
